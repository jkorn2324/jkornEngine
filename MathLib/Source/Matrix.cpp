...
#pragma region matrix4x4
	...
	{
		return CreateEuler(eulers.z, eulers.y, eulers.x, inDegrees);
	}

	Matrix4x4 Matrix4x4::CreateEuler(float yaw, float pitch, float roll, bool inDegrees)
	{
		return CreateRotationX(roll, inDegrees) * 
			CreateRotationY(pitch, inDegrees) * CreateRotationZ(yaw, inDegrees);
	}
	...
	{
		return CreatePersp(fov, width / height, nearPlane, farPlane);
	}

	Matrix4x4 Matrix4x4::CreatePersp(float fov, float aspectRatio, float nearPlane, float farPlane)
	{
		float top = Tan(fov / 2.0f) * nearPlane;
		float bottom = -top;
		float right = top * aspectRatio;
		float left = -top * aspectRatio;
		return CreatePersp(left, right, top, bottom, nearPlane, farPlane);
	}
	...
	void Matrix4x4::Invert()
	{
		...
		invertedScaleMat[0][0] = 1 / (scale.x * scale.x);
		invertedScaleMat[1][1] = 1 / (scale.y * scale.y);
		invertedScaleMat[2][2] = 1 / (scale.z * scale.z);
