...
int                 GameLoop(HINSTANCE& instance, const HWND& window, HACCEL& accelTable);
...
int GameLoop(HINSTANCE& instance, const HWND& window, HACCEL& accelTable)
{
    ...
    DirectXTestProject::Game game;
    if (!game.Initialize(window))
    {
        return 0;
    }

    time_point prevTime = std::chrono::high_resolution_clock::now();
    MSG msg;
    while (true)
    {
        ...
        // Listens for when the user quits the project.
        ...
        // Update the time.
        {
            ...
            do
            {
                ...
                deltaTime = (float)diff.count() * 0.000000001f;
            } while (deltaTime <= MAX_TIME_BETWEEN_FRAMES);
...
//
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow, HWND& outWindow)
{
